---
title: "Lesson 26 — Breastfeeding"
nav_order: 3
parent: "Phase 6: The Newborn"
---

[← Home]({{ '/' | relative_url }}){: .btn .btn-outline }

---

# Lesson 26: Breastfeeding

{: .important }
> General education, not medical advice. If feeding is painful or you're worried
> about the baby getting enough, ask a provider or lactation consultant early —
> help makes a huge difference. **Fed is what matters** (Lesson 27 covers formula).

{: .note }
> **Words to know**
> - **colostrum** (koh-LOSS-trum) — the thick, golden first milk, rich in antibodies (small amounts are enough at first).
> - **latch** — how the baby attaches to the breast; a good, deep latch is comfortable and effective.
> - **supply and demand** — milk production rises to match how much the baby feeds.
> - **cluster feeding** — periods of very frequent feeding (often evenings), which is normal.
> - **engorgement** — breasts becoming overly full, hard, and sore.
> - **mastitis** (mass-TY-tiss) — a painful breast inflammation/infection needing prompt care.
> - **lactation consultant** — a specialist who helps with breastfeeding.

## Concept

Breastfeeding is natural, but it's also a **learned skill** — for both mother and
baby — and it's often **hard at first**. Milk works on **supply and demand**: the
more the baby feeds, the more milk is made. The two things that make or break the
early days are a **good latch** and **frequent feeding** — and **getting help
early** if it hurts or you're unsure. And the golden rule of this whole topic:
**a fed baby is the goal**; breastfeeding is wonderful, and so is a healthy,
formula-fed baby (Lesson 27).

## Going Deeper

### The early days

- The first milk is **colostrum** — thick, golden, antibody-rich, and produced in
  **small amounts** (a newborn's stomach is tiny, so that's enough).
- Your **milk "comes in"** (increases and changes) around **day 3–5**; before that,
  frequent feeding of colostrum is normal and important.
- Feed **on demand, 8–12 times in 24 hours**, watching cues rather than the clock.
  **Cluster feeding** (lots of feeds close together, often evening) is normal and
  boosts supply — not a sign of low milk.

### A good latch

A comfortable, effective latch: the baby's mouth is **wide open** and takes in a
big mouthful of breast (not just the nipple), chin touching the breast, lips
flanged out. It should feel like tugging, **not sharp pain**. Different **positions**
(cradle, cross-cradle, football/rugby hold, lying down) suit different mothers —
experiment. **Persistent pain or damage means the latch needs fixing** — see a
lactation consultant; it's usually solvable.

### Is the baby getting enough?

Reassuring signs: **enough wet and dirty diapers** (building to ~6+ wet a day after
the first week), **regaining birth weight** by ~2 weeks and steady gain after,
plus a baby who is generally **content and alert**. You *cannot* measure breast milk
by eye, so these output-and-growth signs are how you know — not the "empty" feeling
of the breast.

### Common problems (and that help exists)

- **Sore nipples** — usually a latch issue; get it checked.
- **Engorgement** — overly full breasts; frequent feeding and comfort measures help.
- **Mastitis** — a red, painful, hot area with flu-like feelings/fever; **contact a
  provider promptly** (it's treatable, sometimes needs antibiotics; keep feeding
  unless told otherwise).
- **Low-supply worries** — often unfounded (cluster feeding and fussiness get
  misread); a lactation consultant can assess truly low supply and help.

Breastfeeding difficulty is **common and usually fixable with support** — reaching
out early is a strength, not a failure. **The partner's role:** bring water and
snacks, handle everything else, encourage, and help find/arrange lactation support.

{: .note }
> **"Breast is best" — with kindness**
> Breast milk has real benefits, and support to breastfeed is worth seeking. But
> breastfeeding doesn't work out for everyone, and a **fed, thriving baby** is
> always the priority. If it isn't working despite help, formula or combination
> feeding (Lesson 27) is a good, healthy choice — not a failure.

### Where to start

Do one thing before the baby arrives: **find your local breastfeeding support**
— a lactation consultant, a clinic, or a helpline — and save the contact in your
phone.

That sounds like a small administrative task and it is the single highest-value
preparation in this lesson. A great many parents hit trouble in the first week,
and the difference between "help is one call away" and "I don't know who to ask
and I'm crying at 4 a.m." is enormous. Early problems are usually fixable, and
usually fixable quickly, by someone who knows what they are looking at.

Agree the **partner's support plan** too: snacks and water within reach of wherever
she feeds, chores absorbed, visitors managed, and the phone call to the lactation
consultant *made by the partner* if she is too depleted to make it.

And read this lesson together, so the expectations are shared and realistic:
breastfeeding is a learned skill, it is often hard at first, support usually
works — and a fed baby is the goal.

---

## Further Reading

| Topic | Source |
|---|---|
| Breastfeeding help & support (NHS) | <https://www.nhs.uk/conditions/baby/> |
| Breastfeeding | <https://en.wikipedia.org/wiki/Breastfeeding> |
| Breastfeeding (MedlinePlus) | <https://medlineplus.gov/breastfeeding.html> |
| Mastitis | <https://en.wikipedia.org/wiki/Mastitis> |

---

## Checkpoint

**Q1.** How does milk supply work, and how often does a newborn feed?

**Your answer:**

<details>
<summary>Show Answer</summary>
<br>
Milk works on <strong>supply and demand</strong> — the more the baby feeds, the more
milk is made. Newborns feed <strong>on demand, about 8–12 times in 24 hours</strong>,
guided by cues rather than the clock; <strong>cluster feeding</strong> (frequent
feeds, often evenings) is normal and boosts supply.
</details>

**Q2.** How do you know a breastfed baby is getting enough milk?

**Your answer:**

<details>
<summary>Show Answer</summary>
<br>
By <strong>output and growth</strong>: enough <strong>wet and dirty diapers</strong>
(building to ~6+ wet a day after the first week), <strong>regaining birth weight</strong>
by ~2 weeks with steady gain after, and a generally content, alert baby. You
<em>can't</em> judge intake by how "full" or "empty" the breast feels.
</details>

**Q3.** What should you do about sharp pain or cracked nipples, and what is mastitis?

**Your answer:**

<details>
<summary>Show Answer</summary>
<br>
Sharp pain/cracked nipples usually mean a <strong>latch problem</strong> — get help
early (a lactation consultant can usually fix it); pain is a signal, not something to
endure. <strong>Mastitis</strong> is a painful, red, hot breast inflammation with
flu-like symptoms/fever — <strong>contact a provider promptly</strong> (it's treatable,
often while continuing to feed).
</details>

---

<!-- nav-next -->
[← Home]({{ '/' | relative_url }}){: .btn .btn-outline }
[Next: Lesson 27 — Formula & combination feeding →](lesson-27-formula-feeding){: .btn .btn-primary }
