---
title: "Lesson 43 — Multiculturalism"
nav_order: 43
parent: "Phase 8: Culture & Identity"
---

[← Home]({{ '/' | relative_url }}){: .btn .btn-outline }

---

# Lesson 43: Multiculturalism

*Words marked ° are explained in plain English in [Words to Know](#words-to-know) at the end of the lesson.*

## Concept

In 1971, Canada became the **first country on Earth to adopt
multiculturalism as official state policy** — Trudeau Sr.
announcing "**multiculturalism**[°](#w-multiculturalism) within a bilingual framework"; the
**Multiculturalism Act (1988)** legislated it; **Charter s.27**
constitutionalized it as an interpretive lens ("this Charter shall
be interpreted in a manner consistent with the preservation and
enhancement of the multicultural heritage of Canadians"). The
origin was characteristically Canadian — a *by-product*: the
1960s B&B Commission (Lesson 42) proposed a bicultural
French-English Canada, and the **"third force"**[°](#w-third-force) (Lesson 16's
Ukrainians loudest among them) objected: *we* are neither, and we
built the Prairies — so bilingual-BI-cultural became
bilingual-MULTI-cultural, and the accident became the identity.

The standard contrast puts the American **melting pot** against the Canadian
**mosaic**[°](#w-mosaic). In the melting-pot story, newcomers arrive and assimilate into a
single national culture. In the mosaic story, newcomers arrive and retain
their heritage, and the nation is the sum of the retained pieces.

Both are myths, and it is worth saying why. Cultural retention in the United
States is in fact high — Chinatowns and Little Havanas thrive there. And
convergence in Canada is also high: by the **second generation**[°](#w-second-generation), language,
intermarriage, and values shift fast toward the mainstream. The behaviour in
the two countries is far more similar than the slogans suggest.

The real difference is **official posture**. The United States never endorsed
retention as national policy; Canada funds it, constitutionalized it in
section 27 of the Charter, and made it the story citizens are told about
themselves. That matters more than the sociology, because official stories
shape who feels they belong — even when they overstate what people actually do.

Underneath the story, though, is a machine, and the machine is what keeps the
consensus stable. It runs like this: **points-based selection** (1967, Lesson
21) produces educated, deliberately chosen migrants, who **concentrate in
three metros** (Lesson 04), and who **naturalize at very high rates** — around
85% (Lesson 32). That converts newcomers into voting citizens, clustered in
precisely the suburban ridings that decide elections. So **every party courts
immigrant voters**, and anti-immigration politics stays electorally suicidal —
which is why the consensus persists.

The Canadian virtue, in other words, has a structure holding it up. That is
not a cynical observation; it is the reason to expect the virtue to survive
pressure that has broken it elsewhere.

Today's demography (Lesson 24): ~23% foreign-born (G7's
highest), visible minorities ~27% nationally — majorities in
Toronto and Vancouver; the 2020s brought the consensus's first
real stress test (housing/volume, L.24/40), and Quebec runs a
declared alternative model (*interculturalism*[°](#w-interculturalism)). This lesson
holds the celebration and the critiques in one hand, per the
course's standing rule.

---

## Going Deeper

### What the policy actually does

Modest machinery, large narrative: heritage-language and
cultural-organization funding (the folkfest layer critics
mock); anti-racism programs; institutional accommodation
norms (uniforms — the Mountie turban decision, 1990, the
emblematic case: the national symbol itself adapted);
citizenship pedagogy (the mosaic as taught self-image,
L.32's ceremony speech); **s.27**[°](#w-s-27)'s interpretive nudge in
Charter cases. The heavier lifting is done by the adjacent
machinery: points selection, **settlement services**[°](#w-settlement-services) (~$2B/yr —
among the world's most funded **integration**[°](#w-integration) programs), s.15
equality and human-rights codes (L.31/33), and birthright
citizenship. Fair summary: multiculturalism is the brand;
the integration state is the product.

### The evidence (does it work?)

The pro column, with numbers: naturalization ~85% (world's
highest tier, L.32); second-generation outcomes among the
OECD's best (university attainment *exceeding*
third-generation Canadians — the reverse of most of
Europe); intermarriage rising generation over generation;
national-identification levels among immigrants at or above
native-born; no sustained anti-immigrant party has ever
held federal weight (the structure above); Toronto and
Vancouver function as majority-minority cities with low
(by world standards) ethnic conflict. The honest asterisks:
selection effects do heavy lifting (comparing Canada's
chosen migrants to Europe's asylum-driven flows flatters
the model — L.16's lesson that generosity was always
selective, modern edition); racialized income gaps persist
into the second generation (especially for Black
Canadians); L.41's frictions tax the credentialed; and the
2020s volume experiment strained the consensus's
load-bearing assumption — that infrastructure (housing)
would keep pace. The consensus bent (targets cut, L.24);
notably, it did not break: the argument stayed about
*numbers and housing*, not about *who* — an important
distinction to preserve when you hear the debate.

### The critiques, seriously stated

- **From the right/cohesionist:** official retention risks
  **parallel societies**[°](#w-parallel-societies), slows the shared-identity formation a
  country needs, and funds difference where it should fund
  common ground; some imported conflicts (diaspora
  politics) land in Canadian streets. Counter-evidence
  above (convergence data), but the concern isn't
  imaginary — it's managed.
- **From the left/materialist:** the folkfest layer —
  "saris, samosas, and steel drums" (the standard critique
  phrase) — celebrates culture while ignoring structure:
  income gaps, credential walls (L.41), policing
  disparities (L.33); multiculturalism as the cheap
  symbolic substitute for expensive material equality.
- **From Quebec — interculturalism:** the deepest critique,
  from Lesson 22's logic: multiculturalism-as-mosaic
  implies no *host* culture — acceptable to an English
  Canada whose culture is continentally secure, existential
  to a French island of 2%: Quebec's declared model instead
  posits a francophone common culture that newcomers join
  (integrate *into* French, per Bill 101's schooling), with
  diversity welcomed inside that frame. Its hard edge is
  the laïcité file (Bill 21, L.31's s.33 case — religious-
  symbol bans defended as integration policy, experienced
  by Muslim women as exclusion); its soft form is arguably
  just multiculturalism with a declared trunk language —
  and the mutual caricature (Quebec-as-intolerant vs
  English-Canada-as-rootless) is itself a two-solitudes
  artifact (L.28's monolith maps, cultural edition).
- **From Indigenous nations:** the framework's quiet
  category error — First Nations, Inuit, and Métis are not
  "cultures in the mosaic" but *founding polities with
  treaties* (L.06/17); filing them under diversity
  flattens nation-to-nation into heritage-festival. Hence
  the standing insistence (L.24's oath addition): three
  founding pillars — Indigenous, French, English — then
  the mosaic atop, not instead.

{: .note }
> **Using the model in conversation**
> The claim "diversity is our strength" is Canada's civic
> liturgy; the sophisticated version you now own: diversity
> plus *selection, settlement funding, citizenship access,
> and electoral inclusion* is the strength — the adjectives
> carry the sentence. Deploy accordingly: against the
> **nativist**[°](#w-nativist), the outcomes data; against the triumphalist,
> the selection asterisk and the L.41 frictions; against
> both, the observation that the consensus is a built
> structure, not a national virtue gene — structures need
> maintenance, and the 2020s were a maintenance call.

---

## Further Reading

| Topic | Source |
|---|---|
| Multiculturalism in Canada | <https://en.wikipedia.org/wiki/Multiculturalism_in_Canada> |
| Canadian Multiculturalism Act | <https://en.wikipedia.org/wiki/Canadian_Multiculturalism_Act> |
| Interculturalism (Quebec) | <https://en.wikipedia.org/wiki/Interculturalism> |
| Demographics of Canada | <https://en.wikipedia.org/wiki/Demographics_of_Canada> |
| Baltej Dhillon (the Mountie turban case) | <https://en.wikipedia.org/wiki/Baltej_Singh_Dhillon> |
| Charter s.27 | <https://en.wikipedia.org/wiki/Section_27_of_the_Canadian_Charter_of_Rights_and_Freedoms> |

---

## Checkpoint

**Q1.** *(citizenship-style)* When did Canada adopt official
multiculturalism, what instruments carry it, and what
accident of history produced it?

**Your answer:**

<details>
<summary>Show Answer</summary>
<br>
1971 — the world's first official multiculturalism policy
(Trudeau Sr.: "multiculturalism within a bilingual
framework"), legislated as the Canadian Multiculturalism Act
(1988) and constitutionalized as an interpretive principle
in Charter s.27 (1982). The accident: the 1960s Bilingualism
and Biculturalism Commission proposed a two-cultures Canada;
the "third force" — especially Prairie Ukrainians and other
Lesson 16 communities — objected that they were neither
British nor French, so BI-culturalism became
MULTI-culturalism: the policy was born as an amendment to a
different policy, then grew into the national self-image.
</details>

**Q2.** What does the integration evidence show, and which
asterisk must accompany it?

**Your answer:**

<details>
<summary>Show Answer</summary>
<br>
Shows: world-leading outcomes — ~85% naturalization uptake;
second-generation educational attainment exceeding
established-Canadian levels (the reverse of most European
patterns); rising intermarriage; immigrant national
identification at native-born levels; no electorally
significant nativist party; functioning majority-minority
metros. Asterisk: selection — the points system chooses
educated, employable migrants, so outcome comparisons with
countries receiving primarily humanitarian flows flatter
the Canadian model; and within the success, persistent
racialized income gaps, credential frictions (Lesson 41),
and policing disparities (Lesson 33) mark the difference
between symbolic inclusion (excellent) and material
equality (incomplete).
</details>

**Q3.** State Quebec's and Indigenous peoples' distinct
objections to the multicultural frame — and why both are
category claims, not diversity objections.

**Your answer:**

<details>
<summary>Show Answer</summary>
<br>
Quebec (interculturalism): a founding NATION cannot be
reclassified as one culture in another nation's mosaic —
Quebec welcomes diversity into a declared francophone
common culture; the objection defends nationhood status
(Lessons 22–23's whole file), not homogeneity. Indigenous
peoples: First Nations, Inuit, and Métis are founding
POLITIES with treaties and constitutional rights (s.35,
Lessons 06/17/24) — not heritage communities; filing them
under multicultural diversity converts nation-to-nation
relationships into festival programming. Both objections
accept diverse societies; both reject the mosaic's flat
ontology — insisting Canada's structure is founding
pillars (Indigenous, French, English) with the mosaic
built atop, which is in fact how the citizenship oath
(L.32) now orders it.
</details>

---

## Words to Know

*Simple definitions and pronunciations for the terms marked ° above.*

- <a id="w-multiculturalism"></a>**multiculturalism** — the official policy that newcomers may keep their heritage; Canada is the sum of its cultures.
- **melting pot** vs <a id="w-mosaic"></a>**mosaic** (moh-ZAY-ik) — the US image (cultures melt into one) vs the Canadian image (distinct tiles forming one picture); both are partly myths.
- <a id="w-interculturalism"></a>**interculturalism** — Quebec's alternative model: diversity welcomed *into* a declared common francophone culture.
- <a id="w-third-force"></a>**"third force"** — 1960s term for Canadians neither British nor French (Ukrainians, Germans, Italians…), whose objection created multiculturalism.
- <a id="w-nativist"></a>**nativist** (NAY-tih-vist) — politics hostile to immigrants; <a id="w-parallel-societies"></a>**parallel societies** — the fear of communities living separately from the mainstream.
- <a id="w-settlement-services"></a>**settlement services** — funded programs (language classes, job help) that integrate newcomers.
- **assimilation** vs <a id="w-integration"></a>**integration** — being absorbed and losing your culture vs joining fully while keeping it.
- <a id="w-second-generation"></a>**second generation** — the Canadian-born children of immigrants (the key measure of whether integration works).
- <a id="w-strawman"></a>**strawman** — misrepresenting an opponent's argument into a weak version so it's easy to knock down.
- <a id="w-s-27"></a>**s.27** — the Charter clause requiring interpretation consistent with Canada's multicultural heritage.
- Names: **Baltej Dhillon** (BAHL-tej DIL-un — the Mountie who won the right to wear a turban, 1990), **laïcité** (lah-ee-see-TAY — Lesson 22).

---

<!-- nav-next -->
[← Home]({{ '/' | relative_url }}){: .btn .btn-outline }
[Next: Lesson 44 — Indigenous Canada Today →](lesson-44-indigenous-canada-today){: .btn .btn-primary }
